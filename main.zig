...
pub fn main(init: std.process.Init) !void {
    ...
    var messages_channel: Channel(ClientMessage) = try .init(io, gpa, 128);

    try group.concurrent(io, world.start, .{ io, gpa, &messages_channel, &db });

    while (true) {
        print("INFO: [Server] Waiting for new connection...\n", .{});
        var stream = try server.accept(io);
        try group.concurrent(io, session.start, .{ io, gpa, &stream, &messages_channel });
    }
}
